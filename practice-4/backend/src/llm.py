...
class MemoryLLM():
    def __init__(self) -> None:
        ...
        "memory": "Учитывая следующие входные данные из ячеек памяти, ответьте на вопрос, если он предоставлен в ячейках памяти, в противном случае просто ответьте без использования ячейек памяти:\n`{text}`\nЯчейки памяти:\n{context}",
        "memory_delimiter": "ЯЧЕЙКА ПАМЯТИ {i}: {query}\n",
        }
    ...
    def memory_response(self, request: str, memory_queries: List[str]) -> Any:
        ...
        for i, query in enumerate(memory_queries):
            context += self.prompt_templates['memory_delimiter'].format(i = i, query = query)

        request = self.prompt_templates['memory'].format(text = request, context = context)
        
        return self.generate(request)

class GigaChatLLM(MemoryLLM):
    def __init__(self) -> None:
        super().__init__()
